...
$(document).ready(function() {
  ...
  $("#" + USER_INFO_DIV_ID).on("session", function() {
    $.post(API_URL + "render_schedule", { term: term, html: scheduleHTML }, function(response) {
      ...
      if (response.success) {
        $("#" + SHARE_BUTTON_ID).off("click");
        $("#" + SHARE_BUTTON_ID).click(function() {
          $("#" + LOADER_ID).show();
          // submit request to server to post student schedule on click
  ...
  $("#" + USER_INFO_DIV_ID).on("session", function() {
    ...
    var renderingData = { show_section: false, }
    // get user's friends in each class
    for (var i = 0; i < classCodes.length; i++) {
      getFriends(createCallback(classCodes[i]), term, classCodes[i]);
    }
  });
});
