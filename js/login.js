  ...
  FB.Event.subscribe('auth.authResponseChange', function(response) {
    ...
    if (response.status === 'connected') {
      ...
      $(document).ready(function() {
        $.getJSON(API_URL + "access", { access_token: respone.authResponse.accessToken }, 
          function(data) {
            if (!data.success) {
...
// Insert the fb-root div
(function(d) {
  var fb_root = d.createElement("div");
  fb_root.id = "fb-root";
  if (d.getElementById(fb_root.id)) {return; }
  d.head.appendChild(fb_root);
})(document);
